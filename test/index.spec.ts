...
describe('Unit testing for typedoc-plugin-versions', function(){
	it('loads and parses options', function(){
		...
		assert.hasAllKeys(options, stubOptionKeys, 'did not parse all "versions" custom options')
	})
	...
	describe('creates browser assets', function(){
		it('creates a valid js string from the sematic groups', function(){
			...
			assert.equal(vUtils.makeJsKeys(semGroups), jsKeys, 'did not create a valid js string')
		})
		it('creates a redirect string for index.html', function(){
			const url = vUtils.getGhPageUrl({url: null});
			assert.equal(vUtils.makeIndex(url), htmlRedirect, 'did not create a valid htm redirect string')
		})
	...
	describe('creates symlinks', function(){
		...
		it('creates all minor version links', function(){
			...
			stubSemanticLinks.forEach(link => {
				...
				assert.isTrue(fs.existsSync(linkPath), `did not create a symlink for ${link}`)
			})
		})
	})
	describe('handle file operations correctly', function(){
		...
		it('maps correct output paths', function(){
			assert.hasAllKeys(dirs, stubPathKeys, 'not all directories returned');
			assert.isTrue(dirs.rootPath.endsWith(stubRootPath), 'root path not resolved correctly');
			assert.isTrue(dirs.targetPath.endsWith(stubTargetPath(version)), 'target path not resolved correctly');	
		})
		it('does not error if .nojekyll is not present', function(){
			assert.doesNotThrow(() => {vUtils.handleJeckyll(dirs.rootPath, dirs.targetPath)})
		})
		it('copies .nojekyll to the document root if exists', function(){
			...
			assert.isTrue(fs.existsSync(path.join(dirs.rootPath, '.nojekyll')), 'did not move .nojekyll')
		})
		it('copies static assets into the target version folder', function(){
			vUtils.handleAssets(dirs.targetPath);
			assert.isTrue(fs.existsSync(path.join(dirs.targetPath, 'assets/versionsMenu.js')))
		})
	})
})